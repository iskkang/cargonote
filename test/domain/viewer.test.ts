...
const template = {
  id: 't', name: 'T', carrier: '中国税関', route: 'TCR', anchorType: 'container_no', minCount: 8, warningText: null, rules: {},
  requiredPhotos: [{ key: 'seal', label: 'シール接写', instruction: '', required: true }],
} as WorkTypeTemplate;

const review: WorkOrderReview = {
  order: { id: 'wo1', customerId: 'c1', templateId: 't', workDate: null, status: 'submitted', assigneeName: null, assigneeContact: null, assigneeEmail: null, shipperLabel: null, plannedContainerType: null, plannedContainerCount: null },
  template,
  customer: { id: 'c1', name: '青島パートナー', contactName: null, phone: null, email: null, contact: null, notes: null },
...
test('builds a viewer manifest with route, customer, labels, and signed urls', () => {
  ...
  expect(m.customer).toBe('青島パートナー');
  expect(m.containers).toHaveLength(1);
  expect(m.containers[0].containerNo).toBe('ABCD1234567');
  expect(m.containers[0].photos[0]).toEqual({ slotKey: 'seal', label: 'シール接写', thumbUrl: 'https://s/seal-t', displayUrl: 'https://s/seal-d', hash: 'h', capturedAt: '2026-07-02T00:00:00Z' });
});
